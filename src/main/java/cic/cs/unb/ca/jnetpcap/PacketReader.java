...
public class PacketReader {
    ...
    //读取网卡抓到的数据包，packet为已拿到的数据包
    public static BasicPacketInfo getBasicPacketInfo(PcapPacket packet, boolean readIP4, boolean readIP6) {
        BasicPacketInfo packetInfo = null;    //IP数据包基本信息

        Protocol protocol = new Protocol();    //协议类型

        if (readIP4) {
            packetInfo = getIpv4Info(packet, protocol);    //分解IPV4数据包头部
